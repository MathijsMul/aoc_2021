...
def read_file(input_path: str):
    ...
    # return [[list(map(int, coords.split(","))) for coords in line.strip().split("->")] for line in fileinput.input(input_path)]
    return np.array(
        [
...
def get_grid(lines):
    # max_x = max(loc[0] for line in lines for loc in line)
    # max_y = max(loc[1] for line in lines for loc in line)
